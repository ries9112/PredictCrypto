...
# REMOVED TO STOP SHARING MESSARI DATA:
...
query <- "SELECT Date as 'DateExtracted', DateTime as 'DateTimeColoradoTimeMST', Name, Price as 'PriceUSD', Exchange FROM shrimpy_prices WHERE Date >= date_sub(now(), INTERVAL 31 DAY) AND Name != '' order by pkDummy desc"


shrimpy_prices <- dbFetch(dbSendQuery(database_connection, query), 750000)

# write.csv(shrimpy_prices, 'data/shrimpy_prices.csv')

library(pins)

#pins::pin(cryptoData,board='github')

pins::pin(shrimpy_prices,board='github')
