...
#'@export
calculate_price_change_past <- function(data, hours_later){ #would be better to take name/unique identifier from user too here
  ...
  data_join <- rename(data_join, sell_price_usd_24h_earlier = 'sell_price_high_bid') # ADJUST FIELD NAME WITH TIDY EVAL

  # join data and overwrite old object
  data_join <- merge(x=data, y=data_join[, c('pkey', 'sell_price_usd_24h_earlier')], by = 'pkey', all.x = T)

  # remove rows without target
  data_join <- data_join[complete.cases(data_join[,'sell_price_usd_24h_earlier']),]
