...
calculate_percent_change <- function (crypto_dataset, units_offset, units=c('hours','days'))
{
  ...
  if (units == 'hours'){
    ...
    #create 24h offset
    crypto_datasetHLater$date_time_utc <- crypto_datasetHLater$date_time_utc - lubridate::hours(units_offset)

    #replace pkDummy
    crypto_datasetHLater$pkDummy <- substr(paste(as.POSIXct(crypto_datasetHLater$date_time_utc,format="%Y-%m-%d"), format(as.POSIXct(crypto_datasetHLater$date_time_utc,format="%H:%M:%S"),"%H")),1,13)

    crypto_dataset$pkDummy <- substr(paste(as.POSIXct(crypto_dataset$date_time_utc,format="%Y-%m-%d"), format(as.POSIXct(crypto_dataset$date_time_utc,format="%H:%M:%S"),"%H")),1,13)
    ...
    crypto_datasetHLater <- dplyr::select(crypto_datasetHLater, price_usd, pkey, date_time_utc) %>%
      dplyr::rename(price_usd_x_hoursLater = price_usd, date_time_utc_x_hoursLater = date_time_utc)
    ...
    #                          max(crypto_dataset$date_time_utc) - (24*60*60 )
    ...
    #create 24h offset
    crypto_datasetHLater$date_time_utc <- crypto_datasetHLater$date_time_utc - lubridate::days(units_offset)
    ...
    crypto_datasetHLater <- dplyr::select(crypto_datasetHLater, price_usd, pkey, date_time_utc) %>%
      dplyr::rename(price_usd_x_daysLater = price_usd, date_time_utc_x_daysLater = date_time_utc)
