...
#'@export
calculate_perc_change_btc_coinmetrics <- function (coin_metrics, enter_hours)
{
  ...
  coin_metricsHLater <- dplyr::select(coin_metricsHLater, price_btc, pkey, date_time_utc) %>%
    #dplyr::rename("price_btc_{{ enter_hours }}_hoursLater" = price_btc, date_time_utc_x_hours_later = date_time_utc)
    dplyr::rename(price_btc_x_hours_later = price_btc, date_time_utc_x_hours_later = date_time_utc)
  ...
  joinedDataset$target_perc_change <- ((as.numeric(joinedDataset$price_btc_x_hours_later) -
                                          as.numeric(joinedDataset$price_btc))/as.numeric(joinedDataset$price_btc)) * 100
