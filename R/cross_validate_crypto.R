...
cross_validate_crypto <- function(data, splits = 5) {
  ...
  for (i in 1:nrow(crypto_data)) {
    ...
    quantiles_test$data[[i]][quantiles_test$data[[i]]$split == splits, 'training'] <-
      'holdout'
  ...
  for (j in 1:nrow(quantiles_train)) {
    for (i in 1:splits) {
      if (j == 1) {
        # if first iteration need to first initialize new objects _temp and _new
        quantiles_train_temp <- quantiles_train
        quantiles_train_new <- quantiles_train_temp

        quantiles_train_temp$data[[j]] <-
          quantiles_train$data[[j]] %>%
          filter(split == i) %>%
          bind_rows(filter(quantiles_test$data[[j]], split == i - 1))

        quantiles_train_new$data[[j]] <-
          bind_rows(quantiles_train_new$data[[j]],
                    quantiles_train_temp$data[[j]])
      }
      else{
  ...
  quantiles_train_new <-
    quantiles_train_new %>% mutate(exchange = 'HitBTC')
  ...
  for (i in 1:nrow(quantiles_train_new)) {
    quantiles_new$data[[i]] <-
      bind_rows(quantiles_train_new$data[[i]], quantiles_test$data[[i]])

    quantiles_grouped$data[[i]] <-
      quantiles_new$data[[i]] %>% nest(-split,-training) %>% spread(training, data)
  }

  # save the un-nested version so the dates can be visualized easier
  cross_validate_show_dates <<- quantiles_new
